...
import akka.cluster.typed.{Cluster, ClusterSingleton, Join, SingletonActor, Subscribe}
...
case class MainDistributed(port: Int) extends SimpleSwingApplication:
  ...
  override def top: Frame =
    ...
    val system = ActorSystem(Behaviors.setup[Nothing] { context =>
      ...
      var gameTimer: Option[Timer] = None
      distributedManager.setGameEndCallback(() => {
      ...
      val clusterListener = context.spawn(Behaviors.receiveMessage[Any] {
        case MemberUp(member) if member.address == cluster.selfMember.address =>
          ...
          Behaviors.stopped // Stop the listener after initialization
      ...
      cluster.subscriptions ! Subscribe(clusterListener, classOf[MemberUp])
      Behaviors.empty
    }, "AgarCluster", config)
